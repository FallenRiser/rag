...
class EmbeddingFactory:
    ...
    def _resolve_device(device: str) -> Any:
        """Resolve device string to Haystack ComponentDevice."""
        try:
            from haystack.utils import ComponentDevice
            if device == "auto":
                return ComponentDevice.resolve_device(None)
            return ComponentDevice.from_str(device)
        except ImportError:
            return device  # older Haystack versions accept plain strings
